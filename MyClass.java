class MyClass {

  void add(int x, int y)  //adding two integers 
  {
<<<<<<< HEAD
    System.out.println("Sum of two numbers: " +(x+y));
  }

  void remains(int x,int y) //Remains value
  {
    System.out.println("Remains of two Number is : " +(x%y));
  }

  void Reaminder(int x,int y) //Remainder of the number.
  {
    System.out.println("Remainder of of the value : "+(x%y));
  }
=======
    System.out.println("Sum of two numbers: " +x+y);
     
  }

  void mul(int x,int y)//multiplying two integers
  {
    System.out.println("Product of two numbers: "+(x*y));
  }
  
  void mul1(int x,int y,int z)//multiplying two integers
  {
    System.out.println("Product of two numbers: "+(x*y*z));
  }


  Void Subtraction(int a,int b)
  {
	  System.out.println("Sum of two numbers: "+ (a-b));
  }
  void Subtraction(int c,int d,int e)
  {
	  System.out.println("Sum of two numbers: " + (c-d-e));
  }
  void add(int x, int y, int z){
	System.out.println("sum of three numbers is : "+(x+y+z));
   }
  void add(int a, int b, int c, int d){
     System.out.println("the sum of Four number is : "+(a+b+c+d));
   }
>>>>>>> 744e27af981391abacf6f0e4b4d6660218e5ded2

}
